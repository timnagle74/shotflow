                ...
                <CardContent>
                  <div className="border rounded-md overflow-hidden max-h-[500px] overflow-y-auto">
                    <table className="w-full text-xs">
                      ...
                      <tbody className="divide-y divide-border">
                        {/* Use same logic as filmScribeToShots: compare valid events vs valid locators */}
                        {(() => {
                          const validEvents = filmscribeResult.events.filter(
                            e => e.clipName !== null && e.vfxShotCode !== null && !e.clipName?.startsWith('Opt')
                          );
                          const validLocators = filmscribeResult.locators.filter(
                            l => l.vfxShotCode !== null && l.clipName !== null && !l.clipName?.startsWith('Opt')
                          );
                          const useEvents = validEvents.length >= validLocators.length && validEvents.length > 0;
                          
                          if (useEvents) {
                            return validEvents.map((event, i) => (
                              <tr key={i} className="hover:bg-muted/30">
                                <td className="p-2 font-mono font-bold text-green-400">{event.vfxShotCode}</td>
                                <td className="p-2 font-mono text-blue-400 text-[10px]" title={event.clipName || undefined}>{event.clipName || '—'}</td>
                            ...
                            return validLocators.map((loc, i) => {
